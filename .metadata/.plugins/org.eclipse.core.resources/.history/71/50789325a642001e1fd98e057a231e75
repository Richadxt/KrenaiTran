...
public class LoginService {
	...
	public Login logInAdmin(LoginRequest request) {
		...
		if(existingAdmin.getPassword().equals(request.getPassword())) {
			...
			Login adminCurrentSession = new Login(existingAdmin.getId(),key,LocalDateTime.now());
			
			loginRepository.save(adminCurrentSession);
			
			return adminCurrentSession;
		}
		else {
